 ...
 */
export function extractDbCredentials(connectionString: string): DbCredentials | null {
    try {
        const url = new URL(connectionString);

        if (url.protocol !== "postgres:" && url.protocol !== "postgresql:") {
            console.error("Invalid protocol. Only 'postgres:' or 'postgresql:' are supported.");
            return null;
        }
        ...
        if (!user || !host || !database) {
            console.error("Connection string is missing required components (username, host, or database name).");
            return null;
        }
