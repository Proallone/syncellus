...
export async function up(db: Kysely<any>): Promise<void> {
    ...
    await db
        ...
        .addColumn("role_id", "uuid", (col) => col.references(`${schema}.roles.id`).notNull())
        .addColumn("scope_id", "uuid", (col) => col.references(`${schema}.scopes.id`).notNull())
        // .addPrimaryKeyConstraint("primary_key", ["role_id", "scope_id"])
        .addForeignKeyConstraint("auth_role_scopes_role_id_fk", ["role_id"], `${schema}.roles`, ["id"], (cb) => cb.onDelete("cascade"))
        .addForeignKeyConstraint("auth_role_scopes_scope_id_fk", ["scope_id"], `${schema}.scopes`, ["id"], (cb) => cb.onDelete("cascade"))
        .execute();
    ...
    await db
        ...
        .addForeignKeyConstraint("auth_user_role_user_id_fk", ["user_id"], `${schema}.users`, ["id"], (cb) => cb.onDelete("cascade"))
        .addForeignKeyConstraint("auth_user_roles_role_id_fk", ["role_id"], `${schema}.roles`, ["id"], (cb) => cb.onDelete("cascade"))
        .execute();
    ...
    await db
        ...
        .addColumn("id", "uuid", (col) => col.primaryKey().notNull())
        .addColumn("user_id", "uuid", (col) => col.references(`${schema}.users.id`).notNull().unique())
    ...
    await db
        ...
        .addColumn("id", "uuid", (col) => col.primaryKey().notNull())
        .addColumn("user_id", "uuid", (col) => col.references("auth.users.id").notNull().unique())
        .addColumn("token_hash", "varchar(64)", (col) => col.notNull())
...
export async function down(db: Kysely<any>): Promise<void> {
    ...
    // For more info, see: https://kysely.dev/docs/migrations
    await db.withSchema(schema).schema.dropView("email_verification_tokens_view").execute();
    await db.withSchema(schema).schema.dropTable("email_verification_tokens").execute();

    await db.withSchema(schema).schema.dropView("password_reset_tokens_view").execute();
    await db.withSchema(schema).schema.dropTable("password_reset_tokens").execute();
