...
export class AuthController {
    ...
    public forgotPassword = async (req: TypedRequest<ForgotPasswordRequestBody>, res: Response) => {
        ...
        this.logger.info({ email, action: "forgot-password" }, "Password reset requested");
    ...
    public getProfile = async (req: Request, res: Response) => {
        const { user } = req;
        this.logger.info({ userId: user.public_id, action: "get-profile" }, "Profile requested");

        const profile = await this.service.findUserByPublicID(user.public_id);
        return sendResponse(res, HttpStatus.OK, { message: "This account information", data: profile, schema: UserInformationResponse });
    };
}
