...
export interface IAuthRepository {
    ...
    selectUserByPublicID(public_id: string): Promise<Selectable<AuthUsers | undefined>>;
    updateUserPassword(id: string, newPassword: string): Promise<Selectable<AuthUsers>>;
    getUserRoles(user_public_id: string): Promise<string[]>;
    getUserScopes(user_public_id: string): Promise<string[]>;

    insertPasswordResetToken(entry: Insertable<AuthPasswordResetTokens>): Promise<Selectable<AuthPasswordResetTokens>>;
    selectPasswordResetTokenByHash(tokenHash: string): Promise<Selectable<AuthPasswordResetTokens>>;
    ...
    insertEmailVerificationToken(entry: Insertable<AuthEmailVerificationTokens>): Promise<Selectable<AuthEmailVerificationTokens>>;
    selectEmailVerificationTokenByHash(tokenHash: string): Promise<Selectable<AuthEmailVerificationTokens>>;
