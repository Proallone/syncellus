...
import {
  ...
  hashPassword,
  sha256,
} from "@syncellus/utils/crypto.ts";
...
export class AuthService implements IAuthService {
  ...
  public verifyAccountEmail = async (token: string) => {
    ...
    const tokenRecord = await this.repo.selectEmailVerificationTokenByHash(
      tokenHash,
    );
  ...
  public issueLoginToken = async (
    user: Express.User & { public_id: string },
  ) => {
  ...
  public performPasswordReset = async (token: string, newPassword: string) => {
    const tokenHash = sha256(token);
    const tokenRecord = await this.repo.selectPasswordResetTokenByHash(
      tokenHash,
    );
