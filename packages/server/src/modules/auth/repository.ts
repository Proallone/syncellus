import type { IAuthRepository } from "@syncellus/modules/auth/types.d.ts";
import type {
...
export class AuthRepository implements IAuthRepository {
  ...
  public insertNewUser = async (
    ...
    return await this.db.insertInto("auth.users").values(user).returningAll()
      .executeTakeFirstOrThrow();
  };

  public verifyUserEmail = async (id: string) => {
    return await this.db.updateTable("auth.users").set({ verified: true })
  ...
  public selectUserByEmail = async (
    ...
    return await this.db.selectFrom("auth.users_view").selectAll().where(
      "email",
      "=",
      email,
    ).executeTakeFirst();
  ...
  public selectUserByPublicID = async (
    public_id: string,
  ): Promise<Selectable<AuthUsers> | undefined> => {
    return await this.db.selectFrom("auth.users_view").selectAll().where(
      "public_id",
  ...
  public updateUserPassword = async (
    ...
    return await this.db.updateTable("auth.users").set({
  ...
  public insertPasswordResetToken = async (
    entry: Insertable<AuthPasswordResetTokens>,
  ) => {
    return await this.db.insertInto("auth.password_reset_tokens").values(entry)
      .returningAll().executeTakeFirstOrThrow();
  };

  public selectPasswordResetTokenByHash = async (tokenHash: string) => {
    return await this.db.selectFrom("auth.password_reset_tokens").selectAll()
      .where("auth.password_reset_tokens.token_hash", "=", tokenHash)
      .executeTakeFirst();
  };

  public deletePasswordResetTokenByID = async (
    id: string,
  ): Promise<DeleteResult> => {
    return await this.db.deleteFrom("auth.password_reset_tokens").where(
  ...
  public deletePasswordResetTokensByUserID = async (
    user_id: string,
  ): Promise<DeleteResult> => {
    return await this.db.deleteFrom("auth.password_reset_tokens").where(
  ...
  public insertEmailVerificationToken = async (
    entry: Insertable<AuthEmailVerificationTokens>,
  ) => {
    return await this.db.insertInto("auth.email_verification_tokens").values(
      entry,
    ).returningAll().executeTakeFirst();
  };

  public selectEmailVerificationTokenByHash = async (tokenHash: string) => {
    return await this.db.selectFrom("auth.email_verification_tokens")
      .selectAll().where(
  ...
  public deleteEmailVerificationTokenByID = async (
    id: string,
  ): Promise<DeleteResult> => {
    return await this.db.deleteFrom("auth.email_verification_tokens").where(
  ...
  public deleteEmailVerificationTokensByUserID = async (
    user_id: string,
  ): Promise<DeleteResult> => {
    return await this.db.deleteFrom("auth.email_verification_tokens").where(
