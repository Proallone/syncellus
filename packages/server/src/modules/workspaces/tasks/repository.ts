...
export class TasksRepository implements ITasksRepository {
  ...
  public selectTaskByIDFromDB = async (id: string) => {
    return await this.db.selectFrom("workspaces.tasks").selectAll().where(
      "id",
      "=",
      id,
    ).executeTakeFirst();
  ...
  public insertTasksToDB = async (tasks: Insertable<WorkspacesTasks>[]) => {
    return await this.db.insertInto("workspaces.tasks").values(tasks)
      .returningAll().execute();
  };

  public updateTaskByIDInDB = async (
    id: string,
    task: Updateable<WorkspacesTasks>,
  ) => {
    return await this.db.updateTable("workspaces.tasks").set(task).where(
      "id",
      "=",
      id,
  ...
  public deleteTaskByIDinDB = async (id: string) => {
    return await this.db.deleteFrom("workspaces.tasks").where("id", "=", id)
