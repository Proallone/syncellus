...
import { createUpdateTimestampTrigger } from "@syncellus/hono/database/utils/triggers.ts";
import { schema as auth_schema } from "@syncellus/hono/database/migrations/1753121979263_auth.ts";
...
export async function up(db: Kysely<any>): Promise<void> {
	...
	await db
		...
		.addColumn(
			"user_id",
			"uuid",
			(col) => col.unique().notNull().references(`${auth_schema}.users.id`),
		)
		.addColumn(
			"name",
			"varchar(256)",
			(col) => col.check(sql`LENGTH(name) >= 3`),
		)
		.addColumn(
			"surname",
			"varchar(256)",
			(col) => col.check(sql`LENGTH(surname) >= 3`),
		)
		.addColumn(
			"created_at",
			"timestamptz",
