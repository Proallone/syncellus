...
import { schema } from "@syncellus/hono/database/migrations/1757352818919_workspaces.ts";
...
export async function seed(db: Kysely<DB>): Promise<void> {
  ...
  await db.insertInto(`${schema}.invitation_statuses`).values(
    invitationStatuses,
  ).execute();
  await db.insertInto(`${schema}.timesheet_statuses`).values(timesheetStatuses)
    .execute();
